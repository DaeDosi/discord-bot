...
class ChzzkCog(commands.Cog):
    ...
    # ── /치지직설정 ──────────────────────────────────────────────────────────────
    @app_commands.command(name="치지직설정", description="치지직 알림 설정 방법을 안내합니다.")
    async def chzzk_settings(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title="📺 치지직 알림 설정",
            description=(
                "치지직 알림 설정은 **웹 대시보드**에서 가능합니다.\n\n"
                "👉 https://nexbot.shop/dashboard\n\n"
                "대시보드 로그인 → 서버 선택 → **치지직** 탭에서 스트리머를 추가하세요."
            ),
            color=0x03C75A,
        )
    ...
    # ── /치지직알림테스트 ─────────────────────────────────────────────────────
    @app_commands.command(name="치지직알림테스트", description="DB에 등록된 스트리머로 테스트 알림을 전송합니다.")
    @is_mod_or_admin()
    async def test_chzzk_alert(self, interaction: discord.Interaction):
        ...
        row = await (await db.execute(
            "SELECT chzzk_channel_id, chzzk_name, discord_channel "
        ...
        name      = row["chzzk_name"]
        channel   = interaction.guild.get_channel(row["discord_channel"])

        if not channel:
            return await interaction.followup.send(
                "❌ 등록된 알림 채널을 찾을 수 없습니다. 대시보드에서 채널을 다시 설정해주세요.",
                ephemeral=True,
            )

        chzzk_url = f"https://chzzk.naver.com/live/{chzzk_id}"
        live      = await fetch_live_detail(chzzk_id) or {}
        info      = await fetch_channel_info(chzzk_id) or {}

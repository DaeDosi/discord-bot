...
## 자동 워커는 기본값에서 **strict idle**입니다

`ENABLED=false` **그리고** `DRY_RUN=true`인 동안 — 즉 지금의 운영 기본값 — 시간당 자동
워커는 롤업 견적도 프루닝 견적도 **돌리지 않습니다**. 그 산출물은 로그 한 줄뿐인데
81만 행 집계가 **공유 aiosqlite 연결**을 붙들어 공개 API를 뒤로 밀었기 때문입니다
(실측 2026-08-01: retention 23.4초 동안 `/main`·Rising API 최대 18.4초; 항진식 쿼리를
제거한 뒤에도 남은 `estimate_total`만으로 2,919ms → Rising API 3.0~4.1초).

idle 회차는 `retention_idle` 로그 한 줄만 남기고, **이벤트가 끝나 있으면 최종 성적은
반드시 저장합니다**(그것만은 건너뛰지 않습니다). 실제 삭제는 항상 0입니다.

- **전체 진단이 필요하면** 관리자 엔드포인트 `GET /api/admin/db/retention/report`를
  직접 부르십시오 — 이쪽은 예전 그대로 전체 보고서(`rollup_rows` · `would_delete` ·
  `per_event` · `estimated_reclaim_bytes` · `phase_ms`)를 냅니다.
- **활성화 절차를 시작하면 idle은 자동으로 풀립니다.** 아래 3단계에서 `DRY_RUN=false`로
  바꾸는 순간 워커가 다시 전체 경로(롤업 작성 → 검증)를 돕니다. 그래서 idle 조건은
  `ENABLED=false` 하나가 아니라 **두 플래그를 모두** 봅니다 — 하나만 보면 3단계가
  영영 진행되지 않습니다.

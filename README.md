...
수집을 **세 작업으로 분리**했습니다. 예전에는 '과거 적재'와 '신규 탐색'을 한 작업으로
처리해서, 초기 적재가 정기 주기(4분)에 묶여 수 시간씩 걸리는 구조적 지연이 있었습니다.

| 작업 | 목적 | 실행 방식 | 실측 |
| --- | --- | --- | --- |
| **백필** | 이벤트 시작일까지 과거 전부 적재 | 완료까지 **연속** 처리(별도 워커) | 113페이지 5,650건 → **약 10분** |
| **신규 탐색** | 새로 올라온 클립 발견 | 4분 주기 | 아는 클립만 있는 페이지에서 즉시 종료 → **0.1초** |
| **지표 갱신** | 하트·조회수 변화 수집 | 4분 주기 | 목록을 훑지 않고 카드만 → 80건 **6초** |

**백필**은 커서를 DB(`singcup_backfill_state`)에 저장하므로 **재배포·재시작 후에도 이어서**
진행합니다. 배치(기본 300건)마다 커서와 수치를 저장하고, 락을 주기적으로 연장해 여러
워커가 겹치지 않게 합니다. 카드 조회에 실패한 클립만 `singcup_clip_retry` 큐에 남겨
지수 백오프로 재시도합니다.

**지표 갱신**은 우선순위를 둡니다 — 순위를 직접 좌우하는 **대표 클립은 5분**,
나머지는 45분 주기입니다. 저장해 둔 `videoId`/`recId`로 카드 API만 부르므로 목록을
다시 훑지 않습니다.

| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `SINGCUP_CLIP_INTERVAL_MINUTES` | `4` | 신규 탐색·지표 갱신 주기 |
| `SINGCUP_BACKFILL_BATCH` | `300` | 백필 배치 크기(이 단위로 커서 저장) |
| `SINGCUP_BACKFILL_BATCH_PAUSE` | `3` | 배치 사이 대기(초) |
| `SINGCUP_BACKFILL_LOCK_TTL` | `300` | 백필 락 TTL(주기적으로 연장) |
| `SINGCUP_DISCOVER_MAX_PAGES` | `20` | 신규 탐색 안전 상한(정상은 1~2페이지) |
| `SINGCUP_REP_METRICS_TTL_MINUTES` | `5` | **대표 클립** 지표 갱신 주기 |
| `SINGCUP_METRICS_TTL_MINUTES` | `45` | 나머지 클립 갱신 주기 |
| `SINGCUP_REFRESH_PER_CYCLE` | `80` | 사이클당 지표 갱신 건수 |
| `SINGCUP_CARD_CONCURRENCY` | `4` | 카드 API 동시 요청 수 |
| `SINGCUP_RETRY_MAX_ATTEMPTS` | `3` | 실패 클립 최대 재시도 |
...
```bash
# 시작 — 즉시 202로 응답하고 백그라운드에서 진행한다
curl -X POST 'https://<backend>/api/singcup/clips/backfill' -H 'X-Singcup-Secret: <secret>'

# 처음부터 다시
curl -X POST 'https://<backend>/api/singcup/clips/backfill?restart=true' -H 'X-Singcup-Secret: <secret>'

# 진행 상황 — 처리 건수 / 현재 커서 / 도달 날짜 / 실패 수 / 완료 여부
curl 'https://<backend>/api/singcup/clips/backfill/status' -H 'X-Singcup-Secret: <secret>'
```

백엔드가 뜰 때 미완료 백필이 있으면 워커가 **자동으로 이어서** 돌리므로, 보통은 수동
호출이 필요 없습니다. `status`가 `completed`가 되면 워커는 종료합니다.

> 수집 API를 `for` 문으로 여러 번 연속 호출하는 방식은 쓰지 마세요 — 요청이 겹쳐 같은
> 페이지를 중복 처리하고, 어디까지 처리했는지 알 수 없으며, secret이 셸 기록에 남습니다.
> 위 백필 엔드포인트는 락·커서·상태를 모두 관리합니다.

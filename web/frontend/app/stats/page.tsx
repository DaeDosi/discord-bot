    ...
    <div className="nb-shell bg-bg text-fg">
      ...
      <main className="nb-shell-main w-full px-4 pb-8 pt-3 md:px-6">
        ...
        {/* ── 전이 상태의 높이를 실제 화면과 맞춘다 (CLS) ──────────────────────
            ...
            **빼는 값은 위쪽 헤더보다 커서는 안 된다(UI-X).** 예전에는 220px를 뺐는데
            이 컨테이너 위에 실제로 있는 것은 약 107px뿐이라(1440px 실측) 그 차이만큼
            아래 고지 섹션이 접힘선 **위로** 올라와 있었다 — top=819 / 뷰포트 900,
            즉 81px가 화면 안에 보였다. 데이터가 도착하면 그 81px가 통째로 화면 밖으로
            밀려나 그대로 CLS가 됐다(실측 320px 0.0432 · 390px 0.0744 · 390@150%
            0.1046 · 1440px에서 `v=0.0866` 단일 이동). 빼는 값을 120px로 줄여 고지가
            처음부터 접힘선 아래에서 시작하게 하면 그 이동이 통째로 사라진다
            (실측 320px 0.0017 · 390px 0.0033 · 390@150% 0.0000 · 1440px 0.1617).
            **다시 220px로 늘리지 말 것** — 이 값은 '헤더 높이의 근사치'가 아니라
            '헤더 높이의 상한'이다. */}
